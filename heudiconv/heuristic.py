...
def infotodict(seqinfo):
    ...
    info = {t1w: [], cs_acc5_t1w: [], cs_acc42_t1w: [], t2w: [], t2w_vnav: [], rest: [], rest_abcd: [], piper: [],
    ...
    for s in seqinfo:
        ...
        * image_type
        ...
        if 'rest' in s.protocol_name and s.TR == 2 and s.dim4 == 1 and 'SBRef' in s.series_description:
            info[rest_sbref].append(s.series_id)
        if 'piper' in s.protocol_name and s.TR == 2 and s.dim4 > 3:
            info[piper].append(s.series_id)
        if 'piper' in s.protocol_name and s.TR == 2 and s.dim4 == 1 and 'SBRef' in s.series_description:
