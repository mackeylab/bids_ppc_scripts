...
if [ $# -eq 0 ]; then
echo "USAGE: heudiconv_cmd_IMAfiles.sh <sub_id> <BIDS_output_dir>

Example: heudiconv_cmd.sh CBPDxxx /data/picsl/mackey_group/BIDS/
This runs heudiconv for T1 subjects ONLY, this is suitable for IMA files,
specifically CBPD0154 and CBPD0155 which were lost from the scanner.
...
singularity run -B /data:/mnt --cleanenv /data/picsl/mackey_group/tools/singularity/heudiconv0.5.4.simg -d /mnt/picsl/mackey_group/BPD/dicoms/{subject}/*.IMA -o /mnt/${dir} -f /mnt/picsl/mackey_group/CBPD/bids_ppc_scripts/heudiconv/heuristic.py -s ${subID} --ses 01 -c dcm2niix -b --minmeta;
