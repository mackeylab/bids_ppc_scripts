...
for sub in `find . -maxdepth 2 -mindepth 1 -type d -name "ses-03" | sed -e 's|./||' | sed -e 's|/ses-03||'`
do
echo $sub
if [ -f /data/picsl/mackey_group/CBPD/CBPD_bids/derivatives/mriqc_fd_1_mm/${sub}_ses-03_run-01_T1w.html ]; then
  echo 'it exists'
  else
