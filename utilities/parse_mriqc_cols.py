...
df = pd.read_csv(join(MRIQC_dir,"group_bold.tsv"),
    sep = '\t')
df=df[['bids_name', 'fd_mean', 'fd_num', 'fd_perc', 'size_t']]
df = df.rename(columns={'fd_mean':'MRIQC mean FD', 'fd_num':'MRIQC # outliers > '+threshold+' mm',
    'fd_perc':'MRIQC % outliers > '+threshold+' mm', 'size_t':'MRIQC # of vols'})
